...
while game_on:
    ...
    if len(player_two.all_cards)==0:
        print('Player2 is out of all cards player 1 wins \n')
        game_on=False
        break
    playerone_cards=[]
    ...
    while(at_war):
        if(playerone_cards[-1].value>playertwo_cards[-1].value):
            ...
            if(len(player_one.all_cards)<5):
                print("player one ran out of cards player 2 wins!!")
                game_on=False
                break
            elif(len(player_two.all_cards)<5):
                print("Player Two ran out of cards player 1 wins!!")
                game_on=False
                break
            else:
                for i in range(5):
                    playerone_cards.append(player_one.remove_one())
                    playerone_cards.append(player_two.remove_one())
